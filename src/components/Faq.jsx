...
const Faq = () => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 w-11/12 mx-auto justify-between items-center">
      {faqData.map((item, index) => (
        <div
          key={index}
          className="bg-blue-100 border rounded-md p-4 mb-4 font-libreaskerville"
        >
          <button
          ...
          {openIndex === index && (
            <p className="mt-2 text-gray-800 border-t-2 border-black p-4 sm:text-base lg:text-lg">
              {item.ans}
            </p>
          )}
        </div>
